        ...
        $('#rootwizard').bootstrapWizard({
            onTabShow: function(tab, navigation, index) {
                ...
                if(index == 1)
                {
                    $.ajax({
                      method: 'get',
                      url: '{{ url('fiscus/invoiceprices') }}/' + $('#invoiceproduct').val(),
                ...
                else if(index == 2 )
                {
                    ...
                    $.ajax({
                         method: 'get',
                         url: '{{ url('fiscus/allinvoicelines') }}/' + $('#invoiceproduct').val(),
                         success: function(data){
                           $.each(data, function(i, item){
                    ...
                    if($('#invoiceprice').val())
                    {
                        $.ajax({
                             method: 'get',
                             url: '{{ url('fiscus/specificinvoicelines') }}/' + $('#invoiceprice').val(),
                             success: function(data){
                                $.each(data, function(i, item){
                                       $("#"+item.member_id).prop("checked", true);
                       ...
                       <div class="row">&nbsp;</div>
                            <table class="table table-bordered table-striped" id="members">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td width="5%" align="center"><input type="checkbox" class="checkbox" name="member[]" value="{{ $m->id }}" id="{{ $m->id }}" > </td>
                                        <td>{{ $m->firstname }}</td>
                                        <td>{{ $m->lastname }}</td>
                                    </tr>
                                    @endforeach
