...
class FiscusController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (! $v->passes()) {
            ...
            $invoiceproduct->name = $request->input('finalproductname');
            ...
            $invoiceproductprice->price = $request->input('finalpriceperperson');
            $invoiceproductprice->description = $request->input('finalproductdescription');
            $invoiceproductprice->save();

            $i = 0;
            foreach ($request->input('member') as $m) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (! $v->passes()) {
            ...
            if ($request->exists('isupdate')) {
                $update = 'updated';
                $invoiceproductprice = InvoiceProductPrice::find($request->input('isupdate'));
